...
if not (_is_npu or _is_hip):
    try:
        ...
        # Fallback if sgl_kernel cannot be loaded (e.g., missing CUDA libraries)
        # This allows CPU-only inference to proceed
        import warnings
        warnings.warn(
            f"Could not load sgl_kernel: {e}. "
            "This may impact performance on GPU, but CPU inference should work. "
            "To fix, ensure CUDA libraries are installed.",
            RuntimeWarning
        )
        # Define a dummy silu_and_mul that won't be used on CPU anyway
        silu_and_mul = None
...
# Disable torch.compile for CPU to avoid importing networkx/bz2
try:
    @torch.compile
    ...
    # Fallback if torch.compile fails (e.g., missing bz2 for networkx)
    def _cast_to_e8m0_with_rounding_up(x: torch.Tensor) -> torch.Tensor:
        temp = x.to(torch.float32).view(torch.int32)
        exp = torch.bitwise_right_shift(temp, 23)
        mant = torch.bitwise_and(temp, 0x7FFFFF)
