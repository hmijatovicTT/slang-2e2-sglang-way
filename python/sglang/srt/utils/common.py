...
def support_triton(backend: str) -> bool:
    return backend not in ["torch_native", "intel_amx", "ttmetal"]
...
def get_available_gpu_memory(
    ...
    if device == "cuda":
        ...
        # TT-Metal is not a PyTorch device, use system available memory
        total_free_memory = psutil.virtual_memory().available
        n_numa_node: int = len(get_cpu_ids_by_node())
        free_gpu_memory = round(total_free_memory / n_numa_node, 3)
...
def get_device(device_id: Optional[int] = None) -> str:
    """Return compute device string.

    Prefer accelerators if present, otherwise use plain CPU.
    Never raise if no accelerator is found.
    """
    import torch
    ...
    if is_habana_available():
        try:
            ...
            logger.warning(
                "Habana frameworks detected but failed to import HPU; falling back to CPU."
            )

    # Final fallback: CPU, even if is_cpu() is false
    logger.warning(
        "No accelerator (CUDA, XPU, NPU, HPU) is available, using CPU device."
    )
    return "cpu"
'''
