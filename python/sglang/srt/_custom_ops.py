...
if not is_hip() and not is_npu():
    if sgl_kernel is not None:
        ...
        # Stubs when sgl_kernel is not available
        def init_custom_ar(*args, **kwargs) -> int:
            raise RuntimeError("sgl_kernel not available")

        def all_reduce(*args, **kwargs) -> None:
            raise RuntimeError("sgl_kernel not available")

        def dispose(*args, **kwargs) -> None:
            pass

        def meta_size() -> int:
            return 0

        def register_buffer(*args, **kwargs) -> None:
            pass

        def get_graph_buffer_ipc_meta(*args, **kwargs) -> Tuple[List[int], List[int]]:
            return [], []

        def register_graph_buffers(*args, **kwargs) -> None:
            pass
    ...
    if sgl_kernel is not None:
        ...
        def all_reduce_reg(*args, **kwargs) -> None:
            raise RuntimeError("sgl_kernel not available")

        def all_reduce_unreg(*args, **kwargs) -> None:
            raise RuntimeError("sgl_kernel not available")
        ...
        def allocate_meta_buffer(*args, **kwargs):
            return None

        def get_meta_buffer_ipc_handle(*args, **kwargs):
            return None

        def init_custom_qr(*args, **kwargs) -> int:
            raise RuntimeError("sgl_kernel not available")

        def qr_get_handle(*args, **kwargs):
            return None

        def qr_open_handles(*args, **kwargs) -> None:
            pass

        def qr_all_reduce(*args, **kwargs) -> None:
            raise RuntimeError("sgl_kernel not available")

        def qr_destroy(*args, **kwargs) -> None:
            pass
