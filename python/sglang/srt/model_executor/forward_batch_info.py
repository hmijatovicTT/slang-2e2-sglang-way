...
# from sglang.srt.layers.attention.nsa.utils import NSAContextParallelMetadata  # DISABLED: Class doesn't exist
...
_is_npu = is_npu()
# Disable torch.compile on CPU to avoid importing networkx/bz2
_disable_torch_compile = _is_npu or not torch.cuda.is_available()
...
class ForwardBatch:
    ...
    nsa_cp_metadata: Optional[dict] = None
    ...
    def init_new(
        ...
        # Map unsupported torch device 'tt' to CPU for metadata tensors
        try:
            device_str = str(device)
        except Exception:
            device_str = getattr(device, "type", "cpu")
        meta_device = "cpu" if device_str == "tt" else device

        if batch.extend_input_logprob_token_ids is not None:
            ret.extend_input_logprob_token_ids_gpu = (
                batch.extend_input_logprob_token_ids.to(meta_device, non_blocking=True)
            )

        if enable_num_token_non_padded(model_runner.server_args):
            ret.num_token_non_padded = torch.tensor(
                len(batch.input_ids), dtype=torch.int32
            ).to(meta_device, non_blocking=True)
        ...
        if ret.forward_mode.is_idle():
            ret.positions = torch.empty((0,), dtype=torch.int64, device=meta_device)
...
@torch.compile(dynamic=True, backend=get_compiler_backend(), disable=_disable_torch_compile)
