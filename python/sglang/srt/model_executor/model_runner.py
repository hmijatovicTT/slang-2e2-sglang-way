...
class ModelRunner:
    ...
    def __init__(
        ...
        self.device = server_args.device                  #cuda,cpu,....metal....
        self.gpu_id = gpu_id
        ...
        self.server_args = server_args                  # attention backend flag from starrting server
        ...
        os.environ["HF_MODEL"] = server_args.model_path
        ...
        if (
            ...
            self.attention_layers = []     #for every layer in attention model
    ...
    def init_torch_distributed(self):
        ...
        if not self.is_draft_worker:
            ...
            # Workaround for TT-Metal: pass "cpu" as device to initialize_dp_attention
            # because "tt" is not a valid PyTorch device.
            # The metadata buffers created there are fine on CPU.
            original_device = self.server_args.device
            if self.device == "tt":
            ...
            if self.device == "tt":
                self.server_args.device = original_device
    ...
    def load_model(self):
        ...
        with self.memory_saver_adapter.region(
            ...
            logger.info(
            f"self.model class: {type(self.model).__name__}"
            )
    ...
    def init_memory_pool(
        ...
        if self.server_args.attention_backend == "ascend":
            ...
            # Mirror vLLM TT KV sizing: set num_blocks via heuristic and per-device heads.
            try:
                import os
                import math
                import ast
                ...
                # Per-device KV heads (TT model handles TP internally similar to vLLM)
                total_kv_heads = self.model_config.get_total_num_kv_heads()
                per_device_kv_heads = max(1, total_kv_heads // max(1, min(num_devices_per_model, total_kv_heads)))
                ...
                if (("Llama-3.1-8B" in model_path) and num_devices_per_model == 1 and is_wormhole):
                    max_tokens_all_users = 65536
                else:
                    max_tokens_all_users = 131072
                # Account for worst-case batch touching new blocks
                max_batch = self.server_args.max_running_requests or 32
                max_tokens_all_users += self.page_size * max_batch
                num_tt_blocks = math.ceil(max_tokens_all_users / self.page_size)
                # Override tokens capacity with heuristic blocks
                self.max_total_num_tokens = int(num_tt_blocks * self.page_size)

                # Info log for visibility
                log_info_on_rank0(
                    logger,
                    f"[TT-SGLANG] KV sizing: blocks={num_tt_blocks}, page_size={self.page_size}, tokens={self.max_total_num_tokens}, per_device_heads={per_device_kv_heads}"
                )
            except Exception as e:
                logger.warning(f"[TT-SGLANG] KV sizing heuristic failed: {e}. Using profiled tokens={self.max_total_num_tokens} and total_heads.")
                per_device_kv_heads = self.model_config.get_total_num_kv_heads()

            self.token_to_kv_pool = TTKVCache(
                size=self.max_total_num_tokens,
                page_size=self.page_size,
                dtype=self.kv_cache_dtype,
                head_num=per_device_kv_heads,
                head_dim=self.model_config.head_dim,
                layer_num=self.num_effective_layers,
                device=kv_pool_device,
                enable_memory_saver=self.server_args.enable_memory_saver,
                start_layer=self.start_layer,
                end_layer=self.end_layer,
            )
            self.token_to_kv_pool.allocate_on_device(self.model)
        else:
            if self.is_hybrid:
                ...
                self.token_to_kv_pool = MHATokenToKVPool(
                    ...
                    page_size=self.page_size,
                    dtype=self.kv_cache_dtype,
                    head_num=self.model_config.get_num_kv_heads(attn_tp_size),
                    head_dim=self.model_config.head_dim,
                    layer_num=self.num_effective_layers,
                    device=kv_pool_device,
                    enable_memory_saver=self.server_args.enable_memory_saver,
        ...
        logger.info(
            f"self.token_to_kv_pool class: {type(self.token_to_kv_pool).__name__}"
        )
    ...
    def forward_decode(
        ...
        logger.info(
            f"self.model.forward (decode) to execute"
            )
    ...
    def forward_extend(
        ...
        logger.info(
            f"self.model.forward (extend) to execute"
            )
        return self.model.forward(
            forward_batch.input_ids,
            forward_batch.positions,
