...
try:
    ...
    # Fallback for CPU-only or incomplete builds
    def can_use_hicache_jit_kernel(*args, **kwargs):
        return False
    def jit_transfer_hicache_all_layer(*args, **kwargs):
        pass
    def jit_transfer_hicache_one_layer(*args, **kwargs):
        pass
...
if not (_is_npu or _is_xpu):
    try:
        ...
        # These functions won't be used on CPU
        import warnings
        warnings.warn(
            f"Could not load sgl_kernel.kvcacheio: {e}. "
            "CPU-only inference will proceed without acceleration.",
            RuntimeWarning
        )
