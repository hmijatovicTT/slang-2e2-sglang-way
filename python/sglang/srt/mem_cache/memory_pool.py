...
Supported KVCache implementations:
- MHATokenToKVPool: Multi-Head Attention (standard Transformer)
- MLATokenToKVPool: Multi-head Latent Attention
- NSATokenToKVPool: Non-Sparse Attention
- DoubleSparseTokenToKVPool: Double-Sparse attention pattern
- TTKVCache: Custom device/accelerator (TensorTrain, custom hardware, etc.)
  - Override _create_cpu_buffers(), set_kv_buffer(), and get_cpu_copy()/load_cpu_copy()
  - See TTKVCache class for template implementation
"""
...
class ReqToTokenPool:
    ...
    def __init__(
        ...
        alloc_device = device
        # Handle "tt" device string or torch.device object
        if str(device).startswith("tt"):
            alloc_device = "cpu"

        with memory_saver_adapter.region(GPU_MEMORY_TYPE_KV_CACHE):
            self.req_to_token = torch.zeros(
                (size, max_context_len), dtype=torch.int32, device=alloc_device
            )
...
class TTKVCache(KVCache):
    """KVCache implementation for TensorTrain/TT-Metal device.
    
    This class provides the interface for storing and managing KV cache
    on a TT-Metal device. Subclass this and override device-specific methods
    to integrate with your custom device.
    """
    ...
    def __init__(
        ...
        self.head_num = head_num
        self.head_dim = head_dim
        self.kv_caches = None  # Will be populated by allocate_on_device

        # For now, using CPU buffers as placeholder until allocate_on_device is called
        self._create_cpu_buffers()
        self._finalize_allocation_log(size)
        

    def allocate_on_device(self, model):
        """
        ...
        kv_cache_shape = (
            self.size // self.page_size,  # num_blocks
            self.head_num,                # num_kv_heads (already adjusted for TP)
            self.page_size,               # block_size
            self.head_dim,                # head_size
        )

        # 2. Allocate KV Cache Tensors on the device
        self.kv_caches = model.allocate_kv_cache(kv_cache_shape, self.dtype, self.layer_num)

        logger.info(
            f"allocate_kv_cache executed"
        )

        # Clear placeholder CPU buffers
        self.k_buffer = None
        self.v_buffer = None

    def _create_cpu_buffers(self):
        """Create temporary CPU buffers as placeholder.
        
        Replace this with actual device memory allocation for your TT device.
        """
        # Placeholder: store as CPU tensors initially
        self.k_buffer = [
            torch.zeros(
                (self.size + self.page_size, self.head_num, self.head_dim),
                dtype=self.dtype,
    ...
    def get_kv_size_bytes(self):
        """Return total size in bytes for K and V caches.
        
        Since the TT device manages memory internally and we don't have direct access
        to the physical memory state, we return a calculated theoretical size based
        on the configured capacity. This satisfies the interface requirements without
        needing to query the device.
        """
        # Calculate theoretical size:
        # size (tokens) * head_num * head_dim * dtype_size * 2 (K + V) * layer_num
        # Note: self.size is total tokens capacity
        
        element_size = self.dtype.itemsize if hasattr(self.dtype, "itemsize") else 2
        total_bytes = (
            self.size * 
            self.head_num * 
            self.head_dim * 
        ...
        # Return as tuple (k_size, v_size) as expected by some callers, or total bytes?
        # Looking at other implementations (MHATokenToKVPool), it returns (k_size, v_size)
        # or sometimes just uses it for logging.
        # Let's return the split size to be safe and consistent.
        half_bytes = total_bytes // 2
        return half_bytes, half_bytes
    ...
    def get_key_buffer(self, layer_id: int) -> torch.Tensor:
        ...
        Returns:
            K cache tensor for the specified layer
        """
        if self.kv_caches is not None:
            # self.kv_caches structure from TT model is [dp_rank][layer_id][k, v]
            # We return the buffer for the first DP rank (rank 0)
            return self.kv_caches[0][layer_id - self.start_layer][0]
        return self.k_buffer[layer_id - self.start_layer]
    ...
    def get_value_buffer(self, layer_id: int) -> torch.Tensor:
        ...
        Returns:
            V cache tensor for the specified layer
        """
        if self.kv_caches is not None:
            # self.kv_caches structure from TT model is [dp_rank][layer_id][k, v]
            # We return the buffer for the first DP rank (rank 0)
            return self.kv_caches[0][layer_id - self.start_layer][1]
        return self.v_buffer[layer_id - self.start_layer]

    def get_kv_buffer(
        self, layer_id: int
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Get both K and V buffers for a specific layer.
        
        Args:
            layer_id: Layer index
            
        Returns:
            Tuple of (K cache, V cache) tensors
        """
        return self.get_key_buffer(layer_id), self.get_value_buffer(layer_id)
    ...
    def set_kv_buffer(
        ...
        """Set KV values at specified locations.
        
        This is called during inference to write new K,V values to the cache.
        
        Args:
            layer: The RadixAttention layer object
            loc: Tensor of indices where to write K,V values
            cache_k: K values to write
            cache_v: V values to write
        """
        # This method is required by the KVCache interface, but since the
        # TT device manages the cache internally, we expect this to be a no-op.
        # The SGLang scheduler may still call this.
        raise NotImplementedError(
            "set_kv_buffer is not expected to be called in the TT integration."
        )

    def get_cpu_copy(self, indices):
        """Export KV cache values to CPU for offloading/checkpointing."""
        raise NotImplementedError(
            "get_cpu_copy is not expected to be called in the TT integration."
        )

    def load_cpu_copy(self, kv_cache_cpu, indices):
        """Load KV cache values from CPU (from offloaded storage)."""
        raise NotImplementedError(
            "load_cpu_copy is not expected to be called in the TT integration."
        )
